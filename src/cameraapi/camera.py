...
class Camera:
    """
    A class to interface with a camera.
    This class opens the camera in the same process as the code is running from.

    :::warning
    If you want to open the camera in another process, you can use the [MultiprocessCamera](#MultiprocessCamera) class.
    :::
    ...
    Example:
        ...
        camera = Camera(show=True, track_markers=True, compute_point_cloud=True)
...
#region Methods
    ...
    def calibrate(self):
        """
        Calibrate the camera. You need to set up  in the calibration configuration before calling this method.
            ...
            with self._lock:
                ...
                if self._tracking:
                    ...
                    for p_camera in self._camera.trackers_pos:
                        p_ = [p_camera[0], p_camera[1], p_camera[2], 0, 0, 0, 1]
                        self._trackers_pos.append(p_[0:3])
                    logger.debug(f"Trackers positions in camera frame: {self._camera.trackers_pos}, converted to  frame: {self._trackers_pos}")
                if self._compute_point_cloud:
